...
public class RoomController {
    ...
    // ✅ Add new room with Cloudinary image upload
    @PostMapping("/add")
    @PreAuthorize("hasAuthority('ADMIN')")
    public ResponseEntity<Response> addNewRoom(
            @RequestParam(value = "photo", required = false) MultipartFile photo,
        ...
        if (photo == null || photo.isEmpty() || roomType == null || roomType.isBlank() || roomPrice == null || roomDescription == null || roomDescription.isBlank()) {
            Response response = new Response();
            response.setStatusCode(400);
            response.setMessage("Please provide all required fields (photo, roomType, roomPrice, roomDescription)");
            return ResponseEntity.status(response.getStatusCode()).body(response);
        }
    ...
    // ✅ Update room details (with optional image update)
    @PutMapping("/update/{roomId}")
    @PreAuthorize("hasAuthority('ADMIN')")
